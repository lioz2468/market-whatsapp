...
_OUTPUT_RULE = """
⚠️ הנחיית פלט - חובה:
אתה מנסח הודעת WhatsApp מוכנה לשליחה. החזר אך ורק את טקסט ההודעה הסופית.
אסור לחלוטין: שאלות ("האם", "?"), הערות ("עדכון:", "כדאי לוודא"), פנייה אלי, רשימות ממוספרות (1. 2. 3.).
אם חסרים נתונים - נסח על בסיס מה שיש. אם אי אפשר לנסח - החזר את המחרוזת הריקה בלבד."""
...
- מקף ארוך "—" (em dash) — אסור לחלוטין. תמיד תשתמש במקף קצר "-" רגיל""" + _OUTPUT_RULE
...
async def _compose_one(
    ...
    async with semaphore:
        try:
            ...
            if not _is_valid_message(text):
                if attempt <= 2:
                    print(
                        f"[composer] WARNING: response looks like dialogue (attempt {attempt}), retrying — {_label}",
                        file=sys.stderr,
                    )
                    ...
                    print(
                        f"[composer] WARNING: response still invalid after {attempt} attempts, skipping — {_label}",
                        file=sys.stderr,
                    )
                    result.message = ""
                    return
